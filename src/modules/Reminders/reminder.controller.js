...
export const createReminder = async (req, res, next) => {
  try {
    ...
    if (!prescription) {
      return next(new Error("No prescription found for the logged-in user and specified medicine", { cause: 404 }));
    }
...
export const deleteReminder = async (req, res, next) => {
  const { reminderId } = req.query;
    ...
    if (!prescription) {
      return next(new Error("You are not authorized to delete this reminder", { cause: 403 }));
    }
...
export const updateReminder = async (req, res, next) => {
  const { reminderId } = req.query;
  const { reminderMsg, startDate, isTaken, frequency, dosage, duration } = req.body;
    ...
    if (!prescription) {
      return next(new Error("You are not authorized to update this reminder", { cause: 403 }));
    }
...
export const getAllReminders = async (req, res, next) => {
    ...
    const medicineIds = prescriptions.flatMap(prescription => prescription.medicineId);

    // Find all reminders with medicineIds from the prescriptions
    const reminders = await reminderModel.find({ medicineId: { $in: medicineIds } });
...
export const searchReminders = async (req, res, next) => {
  try {
    const { searchQuery, startDate, endDate, frequency, dosage, duration } = req.body;
